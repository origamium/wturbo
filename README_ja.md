...
Git worktree をベースにした CLI ツールで、ブランチごとに独立した作業ディレクトリを提供します。`.env` の自動コピー、ポート再マッピング、Docker Compose 環境の分離、`node_modules` のような重いディレクトリの symlink 化までを一括で面倒見ます。
...
6. **Start command** — `start_command` 設定があれば、新しい worktree 内で `/bin/sh` 経由で実行。

`wtb remove <branch>` は逆順で動作: `docker compose down`(`end_command` 未設定時) → `end_command` → `git worktree remove`。
